...
const { pushLocationMetric } = require('../utapi/utilities');
...
const data = {
    ...
    delete: (objectGetInfo, log, cb) => {
        ...
        _retryDelete(clientGetInfo, log, 0, err => {
            ...
            return pushLocationMetric(objectGetInfo.dataStoreName,
            objectGetInfo.size, 'decrby', log, err => {
                if (err) {
                    log.error('Utapi error updating location metric', {
                        error: err,
                        key: objectGetInfo.key,
                        method: 'pushLocationMetric' });
                }
                return callback(err);
            });
        });
    ...
    uploadPartCopy: (request, log, destBucketMD, sourceLocationConstraintName,
      ...
      callback) => {
        ...
        const locationTypeMatch =
            config.backends.data === 'multiple' && srcType === dstType &&
