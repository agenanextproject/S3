    ...
    return async.waterfall([
        ...
        function deleteOperation(bucketMD, objectMD, delOptions, next) {
            ...
            if (delOptions && delOptions.deleteData) {
                ...
                const location = backendInfoObj.controllingLC;
                const size = Number.parseInt(objectMD['content-length'], 10);
                return locationStorageCheck(null, size, null, location, log,
                err => {
                    if (err) {
                        return next(err);
                    }
            ...
            console.log('\n------CREATE AND STORE OBJECT COMING UPPPP');
