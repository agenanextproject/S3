 ...
 * @return {undefined}
 */
function locationStorageCheck(newObjSize, prevObjSize, location, log, cb) {
    const sizeLimitGB = config.locationConstraints[location].
        details.sizeLimitGB;
    console.log('\n----NEW OBJ SIZE: ', newObjSize);
    console.log('\n----PREVOBJ SIZE: ', prevObjSize);
    const sizeDiff = newObjSize - prevObjSize;

    if (sizeDiff === 0 || sizeLimitGB === undefined) {
        return cb();
    }
