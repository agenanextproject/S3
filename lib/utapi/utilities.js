...
function pushMetric(action, log, metricObj) {
    const { bucket, keys, dataLocation, byteLength, newByteLength,
...
/**
 * Call the Utapi Client 'listLocationMetric' method with the
 * associated parameters
 * @param {string} location - name of data backend to list metric for
 * @param {object} log - werelogs logger
 * @param {function} cb - callback to call
 * @return {function} - `utapi.listLocationMetric`
 */
function listLocationMetric(location, log, cb) {
    return utapi.listLocationMetric(location, log.getSerializedUids(), cb);
}
 ...
 */
function pushLocationMetric(location, byteLength, command, log, cb) {
    return utapi.pushLocationMetric(location, byteLength, command,
